            ...
            <div class="card">
              ...
              <b-modal @shown="focusMyElement" ref="my-modal" :id="infoModal.id" :title="infoModal.title" @hide="resetInfoModal" hide-footer>
                <form @submit.prevent="editMode ? update() : store()"> 
                  ...
                  <div class="modal-body">
                    <b-form-group id="example-input-group-1" label="Kategori" label-for="kategori">
                      <v-select :options="['Pabrik', 'Non Pabrik']"
                        v-model="$v.form.kategori.$model"
                        :state="validateState('kategori')"
                      ></v-select>
                    </b-form-group>
                  </div>
                  <div class="modal-body">
  ...
  export default {
    ...
    methods: {
     loadData() {
      ...
      validateState(kategori) {
        const { $dirty, $error } = this.$v.form[kategori];
        return $dirty ? !$error : null;
      },
        ...
        this.$v.form.$touch();
          ...
          try {
            ...
            let updated = await axios.put('api/unit/'+id,this.form)
             ...
             this.$swal({
                icon: 'Error',
                title: 'Unit Updated Failed '+e.response.data.errors
              });
