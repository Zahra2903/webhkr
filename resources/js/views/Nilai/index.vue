                ...
                <form @submit.prevent="editMode ? update() : store()"> 
                  <div class="modal-body">
                    <b-form-group id="kriteriagroup" label="Kriteria Id" label-for="kriteria_id">
                      <v-select v-model="selected"  :options="kriterias">
...
import { required, minLength } from "vuelidate/lib/validators";
  ...
  export default {
    ...
    methods: {
     ...
     loadData() {
        axios.get("api/nilai").then((response) => {
      ...
      openModal(tipe, title, button,item) {
        ...
        if(tipe=="edit") {
          ...
          this.selected =item.kriteria_id;
          this.selected={label:item.kriteria,value:item.kriteria_id}
      ...
      async store() {
         this.form.kriteria_id = this.selected.value;
         this.$v.form.$touch();
          ...
          try {
            ...
            let response =  await axios.post('api/nilai',this.form)
             //console.log(response.status);
              if(response.status==200){
                  ...
                  this.$swal({
                    icon: 'success',
                    title: 'Nilai Added successfully'
                  });
                  this.loadData();
              }
          } catch (e) {
      ...
      async update() {
        ...
        this.$v.form.$touch();
          ...
          try {
            let updated = await axios.put('api/nilai/'+id,this.form)
              if(updated.status==200){
                  ...
                  this.$swal({
                    icon: 'success',
                    title: 'Nilai Updated successfully'
                  });
                  this.loadData();
              }
          } catch (e) {
             this.$swal({
                icon: 'Error',
                title: 'Nilai Updated Failed '+e.response.data.errors
              });
      ...
      deleteCategory(id) { 
        this.$swal({
          ...
          if (result.isConfirmed) {
            axios.delete("api/nilai/"+id).then(response => {
              this.$swal(
                  'Deleted!',
